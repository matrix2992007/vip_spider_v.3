...
async function startXeony() {
    ...
    try {
        ...
        const username = user.username ? user.username : "F_1kE";
    ...
    try {
        ...
        if (profilePhotos.photos.length > 0) {
            ...
            await XeonBotInc.replyWithPhoto(ownerPhoto, {
                ...
                reply_markup: {
                    inline_keyboard: [
                        [{ text: 'OWNER', url: "https://t.me/F_1kE" }, { text: 'CHANNEL', url: "https://t.me/flashaudio" }, { text: 'GROUP', url: "https://t.me/HX_Q10" }]
                    ]
                }
            });
        } else {
            ...
            await XeonBotInc.reply(lang.first_chat(BOT_NAME, user.full_name), {
                ...
                reply_markup: {
                    inline_keyboard: [
                        [{ text: 'OWNER', url: "https://t.me/F_1kE" }, { text: 'CHANNEL', url: "https://t.me/+6bvFoa3YDxdlM2U9" }, { text: 'GROUP', url: "https://t.me/hshsjijjh" }]
                    ]
                }
            });
        }
        ...
        await XeonBotInc.reply(lang.first_chat(BOT_NAME, user.full_name), {
            ...
            reply_markup: {
                inline_keyboard: [
                    [{ text: 'OWNER', url: "https://t.me/F_1kE" }, { text: 'GROUP', url: "https://t.me/hshsjijjh" }]
                ]
            }
        });
    }
...
*Hold on it to copy the ID.*`;
    ...
    // Execute XeonTele10.js only if the user is verified
    ...
    bot.telegram.getMe().then((getme) => {
        console.table({
            ...
            "Author": "https://t.me/F_1kE"
        })
    })
    bot.catch((err, ctx) => {
    console.error('Telegraf error', err)
    // ممكن تبعت رسالة للمالك بس ما توقفش البوت
})
   }
   //===================================\\
...
let code = await XeonBotInc.requestPairingCode(phoneNumber, 'DEVNEXUS');
    ...
    if (!Array.isArray(adminIDs) || !adminIDs.includes(userID)) {
        return ctx.replyWithMarkdownV2(
            ...
            `📩 *Send proof to the owner @DGXeon*`, 
            {
                reply_markup: {
                    inline_keyboard: [
                        [{ text: "📢 Telegram Channel", url: "https://t.me/hshsjijjh" }],
...
XeonBotInc.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
        console.log(chalk.blue.bold(`✅ Connected: ${XeonBotInc.user.id.split(":")[0]}`));

        // حط كل await هنا جوه async
        try {
            await XeonBotInc.newsletterMsg(idch, { type: 'follow' }).catch(e => {});
            await sleep(1999);
            const dirents = await readdir('./lib2/pairing/', { withFileTypes: true });
        ...
        console.warn(chalk.yellow('⚠️ Connection closed, trying to reconnect...'));
        try {
